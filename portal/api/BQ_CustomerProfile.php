...
class BQ_CustomerProfile extends BQ_Base {
    ...
    { 
        ...
        foreach ($matches as $fmatch) { 
            ...
            // HACK!
            $space = ' ';
            // OVERRIDE HACK! to get rid of extra space
            $space = '';
            if ($prefix == '(') {
                $prefix = '-';
                $suffix = '';
    ...
    public function get_autopayEnabled() {
        return (string)$this->response->response[0]->customer[0]->automatedPayment[0]->enabled;
    }
    public function get_lastInvoiceAmount() {
        $lastInvoiceAmount = $this->response->response[0]->customer[0]->invoices->invoice[count($this->response->response[0]->customer[0]->invoices->invoice) - 1]->totalDue;

        // NOTE: Assumption is that all balances are in US Dollars
        setlocale(LC_MONETARY, 'en_US');

        return self::money_format_custom('%#1n',floatval($lastInvoiceAmount) * 1.0);
    }

    public function get_lastInvoiceAmountFloat() {
        $lastInvoiceAmount = $this->response->response[0]->customer[0]->invoices->invoice[count($this->response->response[0]->customer[0]->invoices->invoice) - 1]->totalDue;

        return floatval($lastInvoiceAmount);
    }

    public function get_lastInvoiceDueDate() {
        return date("m-d-Y", strtotime((string)$this->response->response[0]->customer[0]->invoices->invoice[count($this->response->response[0]->customer[0]->invoices->invoice) - 1]->dueDate));
    }

    public function get_lastPaymentTransactionDate() {
        return date("m-d-Y", strtotime((string)$this->response->response[0]->customer[0]->payments->payment[count($this->response->response[0]->customer[0]->payments->payment) - 1]->transactionDate));
    }
    public function get_lastNInvoices($numInvoices) {
        $invoices = [];

        // NOTE: Assumption is that all balances are in US Dollars
        setlocale(LC_MONETARY, 'en_US');

        $totalNumInvoices = count($this->response->response[0]->customer[0]->invoices->invoice);

        for ($i = 0; $i < $numInvoices; $i++) {
            if ($totalNumInvoices > $i) {
                $invoiceIndex = (($totalNumInvoices - $i) - 1);

                $invoice = new CustomerInvoice;

                $invoice->invoiceNumber = (string)$this->response->response[0]->customer[0]->invoices->invoice[$invoiceIndex]->id;
                $invoice->invoiceDate = (string)$this->response->response[0]->customer[0]->invoices->invoice[$invoiceIndex]->dateCreated;

                $lastInvoiceAmount = $this->response->response[0]->customer[0]->invoices->invoice[$invoiceIndex]->totalDue;
                $invoice->invoiceAmount = self::money_format_custom('%#1n', floatval($lastInvoiceAmount) * 1.0);
    ...
    public function get_responseJSON() {
        $xml = simplexml_load_string($this->response->response[0]->asXML());
